...
- 支持了 LaTeX 解析（想用typst，但后悔莫及（））
	...
	- 来段类型检查：
	- $$ \frac{\frac{\frac{\frac{f: A \  \to \  B \in f:A \  \to \  B, x:A}{f:A \  \to \  B, x : A \ \vdash \ f : A \  \to \  B}Var \qquad \frac{f: A \  \to \  B \in f:A \  \to \  B, x:A}{f:A \  \to \  B, x : A \ \vdash \ x : A}Var }{f:A \  \to \  B , x : A \  \vdash \  f \  x : B}App}{f: A \  \to \  B \ \vdash \  \lambda x.f \ x \  : \  A \  \to \  B}Lam }{\emptyset \vdash \lambda f.x.f\ x:(A \  \to \  B) \  \to \  A \  \to \  B }Lam  $$
	- 来段从日本公式中总结出的好玩的：
